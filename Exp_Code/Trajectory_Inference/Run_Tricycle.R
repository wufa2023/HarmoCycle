...
res <- anndata::read_h5ad('/remote-home/share/data2/wbl/wbl_cellcycle/CCAF_WT/U5_stage_012.h5ad')
...
sce <- estimate_cycle_position(sce, gname.type = "SYMBOL", species = "human")
sce <- estimate_Schwabe_stage(sce, gname.type = "SYMBOL", species = 'human')

# sce <- project_cycle_space(sce, gname.type = "ENSEMBL", species = "human")
# sce <- estimate_cycle_position(sce, gname.type = "ENSEMBL", species = "human")
# sce <- estimate_Schwabe_stage(sce, gname.type = "ENSEMBL", species = 'human')
...
plot_df$stage <- c(res$obs$`stage`) # Add original stage annotations
plot_df$angles <- sce$tricyclePosition
plot_df$prediction <- sce$CCStage

# Preview and save results
print(head(plot_df))
write.csv(plot_df, '/root/Cycle/HarmoCycle_v1/Fig3/Projection_U5.csv')
